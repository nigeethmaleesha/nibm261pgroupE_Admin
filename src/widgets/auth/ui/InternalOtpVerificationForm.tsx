...
import {
  resendInternalForgotPasswordOtp,
  resendInternalLoginOtp,
  verifyInternalForgotPasswordOtp,
  verifyInternalLoginOtp,
} from "@/src/shared/api/internalAuth.api";
import { ApiError } from "@/src/shared/api/http";
import { useInternalAuth } from "@/src/shared/auth/InternalAuthProvider";
import {
  clearPendingInternalOtp,
  readPendingInternalOtp,
  saveInternalPasswordReset,
  updatePendingInternalOtpTiming,
  type PendingInternalFlow,
} from "@/src/shared/auth/pendingInternalAuth";
...
export function InternalOtpVerificationForm({ flow }: { flow: Extract<PendingInternalFlow, "LOGIN" | "FORGOT_PASSWORD"> }) {
  const router = useRouter();
  const toast = useToast();
  const { setAuthenticatedUser } = useInternalAuth();
  ...
  const startHref = flow === "LOGIN" ? "/staff/login" : "/staff/forgot-password";

  useEffect(() => {
    const pending = readPendingInternalOtp(flow);
    if (!pending) {
  ...
  const expirySeconds = useMemo(() => Math.max(0, Math.ceil((expiresAt - now) / 1000)), [expiresAt, now]);
  const resendSeconds = useMemo(() => Math.max(0, Math.ceil((resendAt - now) / 1000)), [resendAt, now]);
  ...
  const handleVerify = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setIsLoading(true);
      if (flow === "LOGIN") {
        const response = await verifyInternalLoginOtp({ email, otp });
        clearPendingInternalOtp();
      ...
      const response = await verifyInternalForgotPasswordOtp({ email, otp });
      clearPendingInternalOtp();
      saveInternalPasswordReset(response.resetToken);
      toast.success("Code verified. Create your new password.");
      router.replace("/staff/forgot-password/reset");
    } catch (caught) {
      setError(caught instanceof ApiError ? caught.message : "Unable to verify the OTP.");
      setOtp("");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const response = flow === "LOGIN"
        ? await resendInternalLoginOtp({ email })
        : await resendInternalForgotPasswordOtp({ email });
      updatePendingInternalOtpTiming({
      ...
      setError(caught instanceof ApiError ? caught.message : "Unable to resend the OTP.");
    } finally {
