...
function notifySessionExpired() {
  ...
  window.dispatchEvent(new Event("repairflow-internal-session-expired"));
}
...
async function performRefresh() {
  return fetch(apiUrl("/internal/auth/refresh-token"), {
    method: "POST",
    credentials: "include",
    cache: "no-store",
  });
...
async function requestRaw(
  ...
  if (options.skipRefresh || !retryAllowed || response.status !== 401) return response;
  if (path === "/internal/auth/refresh-token") return response;
...
export async function logoutInternalRequest() {
  try {
    const response = await fetch(apiUrl("/internal/auth/logout"), {
      method: "POST",
      credentials: "include",
      cache: "no-store",
    });
